...
#############################################
            if(inted1< target):
            # assign the value of targer into max_node id
              inted1 = target
            ...
            if source in self.node_degree:
                ...
                #self.node_degree[source].append(target)
            else:
                self.node_degree[source]=1
                #self.node_degree[source].append(target)
            if target in self.target:
                self.target[target].append(source)
        ...
        if(inted2>inted1):
            self.max_node_id= inted2
        else:
            self.max_node_id= inted1
...
### Implement your code here
#############################################                
            for key in range(self.max_node_id+1):
                sum_of_value=0
                if key in self.target:
                    inted=self.target[key]
                    for value in inted:
                        sum_of_value+= scores[value]/self.node_degree[value]


                new_scores[key]= (1-damping_factor)*node_weights[key] + damping_factor *sum_of_value
